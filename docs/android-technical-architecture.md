...
- 可观测性：App 内必须有基础行为埋点、错误日志和本地诊断日志，后续接入后端事件上报。
...
- 继续使用 mock repository 跑通核心状态闭环。
- 扫码借使用模拟识别和手动输入兜底，验证已建档 / 未建档两条业务路径。
...
- 真机测试：相机权限、扫码入口、扫码失败、手动输入兜底、返回键、不同 Android 屏幕适配。
- 截图级视觉测试：权限态、扫码中、失败态、已建档确认、未建档注册。

V0.6+：

- APK 人工验收：必须生成可安装 Debug APK 或验收 APK。
- 日志验收：关键操作和错误路径必须能在本地日志或后端事件中查到。
- 数据分析验收：至少能回答用户在哪个页面、哪个动作、遇到什么错误、错误发生前后的关键上下文。

## 9.1 埋点与日志原则

- 所有用户关键行为使用结构化事件记录：事件名、用户、页面、动作、结果、时间、脱敏上下文。
- 错误日志必须包含错误码、模块、操作、可复现上下文和脱敏设备信息。
- IMEI 进入日志前默认脱敏，只保留前 3 位和后 4 位。
- token、密码、邀请码明文不得进入普通日志。
- 本地调试阶段先记录到 Android Logcat 和本地事件队列；后端稳定后再上报 `/api/events`。
