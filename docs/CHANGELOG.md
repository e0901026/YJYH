...
- 类型：流程 / 文档 / GitHub 协作。
- 内容：新增 `docs/codex-handoff.md`，把另一台电脑通过 GitHub clone 项目、在 Codex 中添加项目文件夹、读取上下文、启动后端、构建 APK、模拟器/真机验收、Pencil 原型注意事项和下一步建议写入仓库；`Agent.md` 将该文件列为每次跨机器或新会话继续开发的必读文件。
- 影响范围：`Agent.md`、`docs/codex-handoff.md`。
- 验证：文档已覆盖 clone、Codex 添加文件夹、环境变量、后端启动、Android 构建、模拟器安装、真机 LAN APK、质量门禁和 Git 纪律。
- 用户确认：用户要求通过 GitHub 拉取项目并在新机器 Codex 添加文件夹后继续对话和开发。
- 是否进入开发：否。本轮为跨机器协作流程配置。
